...
if (isset($_POST['email']) &&  isset($_POST['password'])) {
	...
	if (empty($email)) {
		...
		$sql = "SELECT * FROM user WHERE email='$email' AND password='$password'  AND var='Recipient'";
		...
		if (mysqli_num_rows($result) === 1) {
          ...
          $sqlx = "SELECT * FROM user WHERE email='$email' AND var='Recipient'";
		...
		if (mysqli_num_rows($resulty) === 0) {
           ...
           echo "<script>alert('Incorrect Password');window.location.href='login1.html?error=Incorrect password';</script>";
		        exit();
            }
